...
function Navigation() {
  ...
  const isMorePage = ['challenges', 'statistics', 'matches', 'players'].includes(state.currentPage);
  ...
  const mobileBottomBar = `
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 safe-area-bottom">
      <div class="grid grid-cols-5 h-16">
        <button onclick="navigateTo('home')" class="flex flex-col items-center justify-center gap-0.5 ${
          state.currentPage === 'home' ? 'text-indigo-600' : 'text-gray-500'
        }">
          ...
          <div class="grid grid-cols-4 gap-3">
            ...
            ${state.user
              ? `
                ...
                ` : ''}
                <button onclick="closeMobileMore(); handleLogout()" class="flex flex-col items-center gap-1.5 p-3 rounded-xl text-red-600 hover:bg-red-50">
                  ${icons.logout}
                  <span class="text-xs font-medium">Logout</span>
                </button>
              `
              : `
                <button onclick="closeMobileMore(); navigateTo('admin')" class="flex flex-col items-center gap-1.5 p-3 rounded-xl text-indigo-600 hover:bg-indigo-50">
                  ${icons.login}
